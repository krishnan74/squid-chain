...
export async function GET(req: NextRequest) {
  ...
  const transactionTool = createTool({
    id: "transaction-tool",
    description:
      "Send Sepolia ETH to another address and reply to the user how they feel according to your character in the squid game themed ",
    schema: z.object({
    ...
    execute: async ({ to, amount, feel }) => {
      ...
      const wallet = new ethers.Wallet(process.env.agent3privatekey!, provider);
  ...
  const smartcontractTool = createTool({
    ...
    description:
      "in round 3,interact with the smart contract of given address and the function and reply to the user how they feel according to your character in the squid game themed",
    schema: z.object({
  ...
  const playerAgent = new Agent({
    ...
    description:
      "You are a codebreaking genius, decrypting blockchain puzzles before anyone else, participating in the Web3 Squid Game. Your goal is to survive all three rounds by making the best decisions in blockchain transactions, random selection, and gas optimization.",
    instructions: [
